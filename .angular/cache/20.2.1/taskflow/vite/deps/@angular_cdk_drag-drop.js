import { isPlatformBrowser } from './chunk-WK6IF3BY.js';
...
var DragRef = class {
  ...
  resetToBoundary() {
    if (
      ...
      this._boundaryElement &&
      this._rootElement && // check if we are overflowing off our boundary element
      isOverflowingParent(
...
var CdkDropList = class _CdkDropList {
  ...
  _setupInputSyncSubscription(ref) {
    ...
    ref.beforeStarted.subscribe(() => {
      ...
      ref
        .connectedTo(
          siblings.filter((drop) => drop && drop !== this).map((list) => list._dropListRef),
        )
